...
public class StringUtil {
	...
	public static String stackTraceToString(Throwable ex) {
		...
		try {
			...
			logger.error("error occur..exception resource close process!!");
		}
	 ...
	 */
	public static String convertToHtml(String comment) {
		String text = "";
	 ...
	 * @return String 변경된 문자열
	 */
	public static String nvlOrder(String str, String NVLString) {
		if( (str == null) || (str.trim().equals(""))) {
			return NVLString;
		} else {
			return str;
	 ...
	 * @return String 결과문자열
	 */
	public static String getCommonPrefix(Locale locale) {
		StringBuffer sbPrefix = new StringBuffer();
		//언어설정(KOR | ENG | JPN)
		sbPrefix.append("LANG=");
		sbPrefix.append(ConfigMng.getLanguage(locale));
	 ...
	 * @return String 결과문자열
	 */
	public static String getCommonPrefix(User userInfo, Locale locale) {
		StringBuffer sbPrefix = new StringBuffer();
		//언어설정(KOR | ENG | JPN)
		sbPrefix.append("LANG=");
		sbPrefix.append(ConfigMng.getLanguage(locale));
	...
	public static String makeSrcEnv(User userInfo) {
		StringBuffer sbPrefix = new StringBuffer();
	...
	public static String makeDataParam(String ... strValues) {
		...
		for (String strValue : strValues) {
			...
			sb.append(ConfigMng.getValue(IPropertyKey.PROC_DATA_SEP));			
		}		
	...
	/**
	 * 숫자를 세자리마다 콤마를 찍는다.
	 * 소수점 이후의 숫자가 0이더라도 표시
	 * @param level 메뉴레벨
	 * @return String 결과문자열
	 */
	public static String toCommaIncZero(String inValue) {
	 ...
	 * 지정된 자리까지만 소수점 보여준다
	 * @param  target 변경숫자
	 * @return decimapPoint 허용된 자릿수
	 */
	public static String roundDcPo(double target, int decimapPoint) {		
		String pattern = "0.";
		for (int i = 0; i < decimapPoint; i++) {
			pattern = pattern + "#";
		}		
		DecimalFormat df = new DecimalFormat(pattern);
        return df.format(target);
...
//					}
	...
	public static String getZeroDotString(String target) {
		...
		if (!"".equals(StringUtil.nvl(target))) {
			if (ConstKey.SEP_DOT.equals(target.substring(0,1))) {
	...
	public static String getRandomAlphabet(int length) {
	    ...
	    for (int i = 1; i <= length; i++) {
	        char ch = (char) ((Math.random() * 26) + 97); // 97 : 소문자, 65: 대문자
	        sb.append(ch);
	    }
	    return sb.toString();
	}
	 ...
	 * 
	 * */
	public static String getMultipleQtyAndPrice(String qty, String price, String currency){
		BigDecimal totalPrice = BigDecimal.ZERO;
		BigDecimal quantity = BigDecimal.valueOf(Integer.parseInt(qty));
		BigDecimal prices = BigDecimal.valueOf(Double.parseDouble(price));
		totalPrice = totalPrice.add(quantity.multiply(prices));
		
		//원화일때만 소수점 처리 없음
		if("KRW".equals(currency) ||"CC0076".equals(currency) ){
			int idx = totalPrice.toString().indexOf(".");
			if(idx !=-1){
				String ttlPrice = totalPrice.toString().substring(0, idx);
				return ttlPrice;
			}else{
     ...
     * @return String 파일명
     */
    public static String parsingDomain(String url){
      String domain = "";
      URL aURL = null;
      try {
         aURL = new URL(url);
         domain = url.replace(aURL.getPath(), "");
