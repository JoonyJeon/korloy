...
public class DateTimeUtil {
	/** 로그처리 객체 */
    protected static Log logger = LogFactory.getLog(DateTimeUtil.class);    
	/**
    ...
    /**
     * 두날짜의 차이를 리턴한다.
     * @param startDate 날짜1(YYYYMMDD)
     * @param endDate 날짜2(YYYYMMDD)
     * @return int 날짜차이
     */
    public static int getDayDifference(String startDate, String endDate)    {
        ...
        Date endDay;
		try {
			endDay = new SimpleDateFormat("yyyyMMddHHmmss").parse(endDate);
    ...
    /**
     * 두 개월수의 차이를 리턴한다.
     * @param startMon 날짜1(YYYYMM)
     * @param endMon 날짜2(YYYYMM)
     * @return int 개월차이
     */
    ...
    /**
     * 기    능 : YYYYMMDD 형식의 날에 월 수를 더해서 날을 얻는다.
     * @param   YYYYMMDD
     * @param   MM
     */
    public static String getDatePlusMonth(String YYYYMMDD , String month){
        ...
        cGivenDay.add(Calendar.MONTH,iMM);
    ...
    public static long getTimeAndTimeToDay(String date1, String date2) {
    	...
    	//정해진시간에서 현재시간까지의 시간차
    	long lDay = (lSecond - lFirst) / (24*60*60*1000) ;
    ...
    /**
     * 기    능 : Calendar를 YYYYMM 형식의 스트링으로 변환한다.
     * @param   Calendar
     * @return  YYYYMM
     */
    ...
    /**
	 * 날짜 형식의 문자를 만든다. 일반 TEXT를 2020년 02월 15일 형식으로 바꾼다.
	 * @param str 일반 TEXT
	 * @return 날짜 형식 str
	 */
	public static String toHanDateFormat(String str){
		StringBuffer sbDate = new StringBuffer("");
		...
		if(str.length() >= 4){
			sbDate.append(str.substring(0, 4));
			sbDate.append("년 ");
		}
		if(str.length() >= 6) {
			sbDate.append(str.substring(4, 6));
			sbDate.append("월 ");
		}
		if(str.length() >= 8) {
			sbDate.append(str.substring(6, 8));
			sbDate.append("일");
		}
    ...
    /**
     * 현재의 날짜와 시간을 취득한다
     * @return  String 현재날짜날짜(년,월,일 시간,분,초로 구성된 12자리 문자열)
     */
    ...
    /**
     * 현재의 날짜와 시간을 취득한다
     * @return  String 현재날짜날짜(년,월,일 시간,분,초,밀리세컨드로 구성된 6자리 문자열)
     */
    public static String getFullDateTime2 (){
        String msgDate = "";
    ...
    public static ArrayList getDateGap(String stDate, String edDate, String gubun){
    	...
    	Calendar sDate = DateTimeUtil.getDateCalendarType(stDate);
		Calendar eDate = DateTimeUtil.getDateCalendarType(edDate);
		// 시작일자 Calendar를 취득
		...
		if(gubun.equals("day")){
			for(Calendar start = monthStart; monthEnd.compareTo(start) >= 0; start.add(Calendar.DATE, 1)) {
				date.add(DateTimeUtil.getDateStringType(monthStart));
			}	
		}else{
			for(Calendar start = monthStart; monthEnd.compareTo(start) >= 0; start.add(Calendar.MONTH, 1)) {
				date.add(DateTimeUtil.getMonthStringType(monthStart));
			}
		}
