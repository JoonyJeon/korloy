 ...
 * @version 1.0
 */
public class PagingUtil implements IPropertyKey{
	/** 리스트싸이즈의기본값 */
     ...
     * 페이지기본값 초기화
     */
	public PagingUtil(int total, List list) {
    ...
    private void setDefaultInformation(String keyBoard, int nTotal, int cPage){
    	...
    	sbBuf.append(keyBoard);
    	sbBuf.append("PAGE_SIZE");
    	cPageSize = ConfigMng.getValueInt(sbBuf.toString());
    	sbBuf.delete(0, sbBuf.capacity());

    	sbBuf.append(keyBoard);
    	sbBuf.append("PAGESET_SIZE");
    	cBigPageSize = ConfigMng.getValueInt(sbBuf.toString());
    	sbBuf.delete(0, sbBuf.capacity());
    	if (cPageSize == 0) {
