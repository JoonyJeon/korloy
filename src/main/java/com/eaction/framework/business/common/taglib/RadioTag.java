...
public class RadioTag extends TagSupport {
        ...
        try {
        	...
        	if (ConstKey.KEY_YES.equals(init)) {
    			if (ConstKey.KEY_PLACE_BEFORE.equals(place)){
    				...
    				sbRadio.append(initCode);
    				sbRadio.append("\" ");
    				if(checked.equals(initCode) ){
    					sbRadio.append( "checked " );
    				}
    				sbRadio.append(getCommonString());
    				sbRadio.append("<i></i>" );
    				sbRadio.append(initName);
    				sbRadio.append("&nbsp;&nbsp;" );
    				sbRadio.append(format);
        	...
        	if (list != null) {
	    		...
	    		for( int i = 0; i < list.size() ; i++ ){
					...
					if(session_lang.equals("KOR")){
						sbRadio.append(codeInfo.getCodenm_k());
    				}else{
    					sbRadio.append(codeInfo.getCodenm_e());
    				}
	    			sbRadio.append("&nbsp;&nbsp;" );
	    			if (i < (list.size()-1)) {
	    			    sbRadio.append(format);
    ...
    private String getCommonString() {
    	...
    	sbRadio.append(event);
		sbRadio.append(" ");
		if (!"".equals(cssId)){
			sbRadio.append("class=\"");
			sbRadio.append(cssId);
			sbRadio.append("\" ");
		}
		if (!"".equals(style)){
